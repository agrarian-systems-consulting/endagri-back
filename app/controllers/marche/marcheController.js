...
const postMarche = (request, response) => {
  ...
  const postMarcheQuery = 'INSERT INTO fiche.marche(id,id_produit,type_marche) VALUES (DEFAULT, $1,$2)';
  dbConn.pool.query(postMarcheQuery, [id_produit, type_marche], (error, results) => {
    if (error) {
      throw error
    }
...
const putMarcheById = (request, response) => {
  const id_marche = request.params.id;
  const { id_produit,type_marche } = request.body;
  const putMarcheByIdQuery = 'UPDATE fiche.marche SET id_produit=$1, type_marche=$2 WHERE id=$3';
  dbConn.pool.query(putMarcheByIdQuery, [id_produit,type_marche,id_marche], (error, results) => {
    if (error) {
      throw error
    }
