...
test('Doit retourner toutes les fiches techniques', async () => {
  const res = await (0, _supertest.default)(_server.default).get('/fiches').expect(200);
...
test("Doit retourner toutes les fiches techniques d'un seul auteur", async () => {
  const res = await (0, _supertest.default)(_server.default).get('/fiches/?id_utilisateur=1').expect(200);
  expect(res.body.length).toBe(3);
});
test("Doit retourner le contenu d'une fiche technique", async () => {
  const res = await (0, _supertest.default)(_server.default).get(`/fiche/${id_fiche_technique}`).expect(200);
...
test('Doit créer une fiche technique sans activités ni ventes', async () => {
  const res = await (0, _supertest.default)(_server.default).post('/fiche').send({
...
test('Doit créer une fiche technique avec des ventes, des activités et des dépenses associées', done => {
  (0, _supertest.default)(_server.default).post('/fiche').send({
...
test("Doit modifier les informations principales d'une fiche technique", async () => {
  const res = await (0, _supertest.default)(_server.default).put(`/fiche/${id_fiche_technique}`).send({
...
test("Doit refuser la modification d'une fiche technique inexistante", async () => {
  const res = await (0, _supertest.default)(_server.default).put(`/fiche/983409834`).send({
    libelle_fiche: 'Un titre modifié',
    ini_debut: 1,
    ini_fin: 3,
    commentaire: 'Un commentaire sur une fiche modifiée'
...
test('Doit supprimer une fiche technique', async () => {
  const res = await (0, _supertest.default)(_server.default).delete(`/fiche/${id_fiche_technique}`).expect(200);
  expect(res.body.id).toBeDefined();
});
...
beforeAll(done => {
  ...
  _pool.default.pool.query(postFicheQuery, [id_fiche_technique, 65, 'Carottes en sol argileux', 65, 2, 6]).then(() => done());
});
afterAll(done => {
  const deleteQuery = 'DELETE FROM fiche.fiche_technique WHERE id=$1';

  _pool.default.pool.query(deleteQuery, [id_fiche_technique]).then(() => done());
});